export const GSHEETS_AI_PROMPT = `I want to turn a Google Sheet into a one-click invoice generator using Invoicely (https://invoicely.app, a browser-only invoice tool with no backend).

The integration is pure link-building. An Apps Script reads the active row, builds an Invoicely invoice JSON object, and constructs a URL like:

  https://invoicely.app/playground#j=<encodeURIComponent(JSON.stringify(invoice))>
...
2. Write a complete Apps Script (Code.gs) with: buildInvoice(row), shareUrlFor(invoice), openActiveRow() that pops a modal that calls window.open(url), and onOpen() that adds an "Invoicely" menu.
...
- Plain-language schema and share-URL spec for AI agents: https://invoicely.app/llms.txt
- Developer page (schema reference, examples): https://invoicely.app/developers
- Live invoice examples to model after: https://invoicely.app/examples

Constraints from the schema:
- Every key used in lineItems[] rows must also appear in columns[].
- summary[].value is a string or number that I compute in the sheet; Invoicely never calculates totals, formats currency, or applies tax.
